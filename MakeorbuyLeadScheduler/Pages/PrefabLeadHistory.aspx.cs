...
namespace MakeorbuyLeadScheduler
{
    public partial class WebForm5 : System.Web.UI.Page
    {
        ...
        public void Summary()
        {
            ...
            string query = "Select Count(*) FROM Mob_Lead_FollowUp WHERE ClientName = '" + ddl_clientname.Text + "' and LeadNo='" + ddl_leadno.Text + "' and Catagory='Prefabs'";
            int rCnt;
            rCnt = 0;
           ...
           // if (rCnt == 0)
             ...
             if (rCnt > 0)
            {
                string aStr = "Select * from Mob_Lead_FollowUp WHERE ClientName = '" + ddl_clientname.Text + "' and LeadNo='" + ddl_leadno.Text + "' and Catagory='Prefabs' ";
                OdbcDataReader dr;
        ...
        public void DocSummary()
        {
            ...
            Gridviewdatadoc();
            OdbcConnection maincon = dba.GeoDBMainCon();
            string query = "Select Count(*) FROM Mob_Lead_docSubmission WHERE ClientName = '" + ddl_clientname.Text + "' and LeadNo='" + ddl_leadno.Text + "' and Catagory='Prefabs' ";
            int rCnt;
            rCnt = 0;
            OdbcCommand Command1 = new OdbcCommand(query, maincon);
            rCnt = int.Parse((Command1.ExecuteScalar().ToString()));
            ...
            //else if (rCnt > 0)
            {
                string aStr = "Select * from Mob_Lead_docSubmission WHERE ClientName = '" + ddl_clientname.Text + "' and LeadNo='" + ddl_leadno.Text + "' and Catagory='Prefabs' ";
                OdbcDataReader dr;
                OdbcCommand cmd = new OdbcCommand(aStr, maincon);
                dr = cmd.ExecuteReader();
                while (dr.Read())
